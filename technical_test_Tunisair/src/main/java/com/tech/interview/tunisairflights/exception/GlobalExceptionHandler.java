...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> resourceNotFoundException(ResourceNotFoundException ex, WebRequest request) {
        ErrorDetails errorDetails = new ErrorDetails(new Date(), List.of(ex.getMessage()), request.getDescription(false));
    ...
    public ResponseEntity<?> bindExcpetionHandler(BindException ex, WebRequest request) {
        ErrorDetails errorDetails = new ErrorDetails(new Date(), ex.getBindingResult().getAllErrors().stream().map(DefaultMessageSourceResolvable::getDefaultMessage).collect(Collectors.toList()), request.getDescription(false));
